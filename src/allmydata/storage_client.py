...
class StorageClientConfig(object):
    ...
    def from_node_config(cls, config):
        ...
        #strip()方法用于去除字符串两端的空白字符（空格、制表符、换行符）返回新字符串
        enabled_storage_plugins = (
            ...
            if name.strip() # 如果经过 strip() 处理后的 name 非空
        )
...
class StorageFarmBroker(service.MultiService):
    ...
    def get_servers_for_psi(self, peer_selection_index):
        ...
        def _permuted(server):
            ...
            # 返回判断结果，以及这个存储服务器的唯一标识 hash（storage_index + server_seed)
...
# 客户端节点保留对应的存储节点的引用，包含存储节点的相关元信息。
# _rref
@implementer(IServer)
