...
# checkcode RPC transfer data format
DataVector = ListOf(TupleOf(Offset, ShareData))
# 定义checkcoder RPC传输数据接口校验
WriteCheckcodeForShare = DictOf(int, TupleOf(DataVector,
...
class RIBucketWriter(RemoteInterface):
    ...
    def close():
        ...
        """
        return None
    def write_imcheckcode_vectors(storage_index=StorageIndex, checkcode_vectors=WriteCheckcodeForShare):
        return None
    def abort():
...
class IGetdataResults(Interface):
    
    def get_sharemap(self):
        """Returns the IP mapping corresponding to the storage node
        """
    def get_shareDetails(self):
        """Returns information about the properties of the share file
        """
