...
from allmydata.web.sys_status import data_my_nickname, data_my_nodeid, data_total_introducers, \
    data_connected_introducers, data_introducers, servers_info, data_known_storage_servers, \
    data_connected_storage_servers
...
class URIHandler(resource.Resource, object):
    ...
    def render_GET(self, req):
        ...
        if t == b"server_status":
            my_nickname = data_my_nickname(self.client)
            my_nodeid = data_my_nodeid(self.client)

            # introducer
            total_introducers_count = data_total_introducers(self.client)
            connected_introducer_count = data_connected_introducers(self.client)
            introducer_info_list = data_introducers(self.client)

            # server

            server_info_list = servers_info(self.client)
            total_servers_count = data_known_storage_servers(self.client)
            connected_servers_count = data_connected_storage_servers(self.client)
            data = {
                "nickname": my_nickname,
                "nodeid": my_nodeid,
                "total_introducers_count": total_introducers_count,
                "total_servers_count": total_servers_count,
                "connected_introducers_count": connected_introducer_count,
                "connected_servers_count": connected_servers_count,
                "servers_info": server_info_list,
                "introducers_info": introducer_info_list,
            }
...
class RootElement(Element):
    ...
    @renderer
    def introducers_xtable(self, req, tag):
