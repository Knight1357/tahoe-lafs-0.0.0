...
import time
from .ethfactory import EthWorker
...
class Encoder(object):
    ...
    def start(self):
        ...
        self._pdp.init()
        self._pdp.save_A_K(self._storage_index)
        for i in range(self.num_segments - 1):
            d.addCallback(lambda res: self._encode_segment(i, is_tail=False))
    ...
    def _encode_segment(self, segnum, is_tail):
        ...
        def _done_gathering(chunks):
            ...
            # 在这里对segment拆分成的k个block进行纠删码处理成n个
    ...
    def _send_segment(self, shares_and_shareids, segnum):
        ...
        for i in range(len(shares)):
            block = shares[i]
            shareid = shareids[i]
            ccodes = self._pdp.keyGen(block)  # generate checkcode for block
            d = self.send_checkcode(shareid, segnum, ccodes, lognum)
            dl.append(d)
    ...
    def send_checkcode(self, shareid, segment_num, checkcodes, lognum):
        if shareid not in self.landlords:
            return defer.succeed(None)
        sh = self.landlords[shareid]
        lognum2 = self.log("put_checkcode to %s" % self.landlords[shareid],
                           parent=lognum, level=log.NOISY)
        d = sh.put_block_checkcode(segment_num, checkcodes, self._storage_index, shareid)

        def _done(res):
            self.log("put_checkcode done", parent=lognum2, level=log.NOISY)
            return res
    ...
    def get_uri_extension_size(self):
        ...
        uri extension = crypttext_hash + crypttext_root_hash  + share root hash
