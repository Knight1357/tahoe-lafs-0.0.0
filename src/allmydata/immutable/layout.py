...
from allmydata.pdp import Pdp

if PY2:
    from future.builtins import filter, map, zip, ascii, chr, hex, input, next, oct, open, pow, round, super, bytes, \
        dict, list, object, range, str, max, min  # noqa: F401
...
from allmydata.util import mathutil, observer, pipeline, log, pdputil
...
class WriteBucketProxy(object):
    ...
    def __init__(self, rref, server, data_size, block_size, num_segments,
        ...
        self._cblock_count = int(block_size // pdputil.CBLOCK_SIZE)
        self._block_all_checkcode_size = int(self._cblock_count * pdputil.CHECK_CODE_SIZE)

        self._offsets['ccode'] = 0
    ...
    def put_block_checkcode(self, segment_num, data, si, shareid):
        """
        向对应share内的block写checkcode
        @param segment_num: 当前段的序号
        @param data: 数据
        @param si: 存储索引storage index
        @param shareid: 共享文件id
        @return:
        """
        offset = self._offsets['ccode'] + segment_num * self._block_all_checkcode_size
        s = b''.join(data.values())
        return self._write_checkcode(si, shareid, [(offset, s)])
    ...
    # 调用写checkcode的RPC接口
    def _write_checkcode(self, si, shnum, datav):
        ccodes = {}
        ccodes[shnum] = (datav, None)
        return self._pipeline.add(len(datav), self._rref.callRemote, "write_imcheckcode_vectors", si, ccodes)
    ...
    def abort(self):
        return self._rref.callRemote("abort").addErrback(log.err,
                                                         "Error from remote call to abort an immutable write bucket")
...
class ReadBucketProxy(object):
    ...
    def _get_share_hashes(self, _ignore):
        ...
        if size % (2 + HASH_SIZE) != 0:
            raise LayoutInvalid("share hash tree corrupted -- should occupy a multiple of %d bytes, not %d bytes" % (
            (2 + HASH_SIZE), size))
        d = self._read(offset, size)
    ...
    def _get_uri_extension(self, unused=None):
        ...
        def _got_length(data):
            if len(data) != self._fieldsize:
                raise LayoutInvalid("not enough bytes to encode URI length -- should be %d bytes long, not %d " % (
                self._fieldsize, len(data),))
