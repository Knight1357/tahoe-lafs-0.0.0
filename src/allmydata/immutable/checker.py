...
from allmydata.check_results import CheckResults, GetdataResults
...
class Checker(log.PrefixingLogMixin):
    ...
    # getdata ----
    def _format_getdata_result(self, results):
        SI = self._verifycap.get_storage_index()
        verifiedshares = dictutil.DictOfSets()  # {sharenum: set(server)}
        servers = {}  # {server: set(sharenums)}
        
        for verified, server, corrupt, incompatible, responded,bucket in results:
            servers.setdefault(server, set()).update(verified)
            for sharenum in verified:
                verifiedshares.setdefault(sharenum, set()).add(server)
            tbucket = bucket
            tserver = server
        # 判断接受到的share数量来确定文件是否完整

        b = layout.ReadBucketProxy(tbucket, tserver, self._verifycap.get_storage_index())
        d = b.get_uri_extension()
        def _parse_uri_extension(data):
            share_size = mathutil.div_ceil(self._verifycap.size, self._verifycap.needed_shares)
            d = uri.unpack_extension(data)
            segment_size = d['segment_size']
            block_size = mathutil.div_ceil(segment_size,
                                           self._verifycap.needed_shares)
            num_segments = mathutil.div_ceil(self._verifycap.size,
                                             segment_size)
            num_shares = d['total_shares']
            tail_segment_size = self._verifycap.size % segment_size
            if not tail_segment_size:
                tail_segment_size = segment_size
            tail_segment_size = mathutil.next_multiple(tail_segment_size,
                                                       self._verifycap.needed_shares)
            share_detail = {
                ...
                "storage_index": si_b2a(SI)
            }
            return share_detail

        d.addCallback(_parse_uri_extension)

        def collct(shareDetails):
            cr = GetdataResults(sharemap=verifiedshares, shareDetails=shareDetails,version="immutable")
            return cr

        d.addCallback(collct)
        return d
    def getdata(self):
        ds = []
        for s in self._servers:
            ds.append(self._getdata_server_shares(s))
        return deferredutil.gatherResults(ds).addCallback(self._format_getdata_result)

    def _getdata_server_shares(self, s):
        def _curry_empty_corrupted(res):
            buckets, responded = res
            key, v = next(iter(buckets.items()))
            return (set(buckets), s, set(), set(), responded, v)
