...
class EncryptAnUploadable(object):
    ...
    def __init__(self, original, log_parent=None, chunk_size=None):
        ...
        self._encryptor = None # 加密器
        self._plaintext_hasher = plaintext_hasher() # 哈希
    ...
    def _get_encryptor(self):
        """
        storage index就是该文件AES加密器密钥的SHA256哈希值。
        @return: 初始化AES加密器，并且给出文件的storage index.
        """
        if self._encryptor:
...
class CHKUploader(object):
    ...
    def start(self, encrypted_uploadable):
        ...
        eu.set_upload_status(self._upload_status) # 设置上传状态
        d = self.start_encrypted(eu) # 开始加密，文件内容在eu内
    ...
    def locate_all_shareholders(self, encoder, started):
        ...
        storage_broker = self._storage_broker # 当前连接的所有存储节点
        secret_holder = self._secret_holder # 密钥连接者
