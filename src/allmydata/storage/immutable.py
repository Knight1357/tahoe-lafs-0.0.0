...
from allmydata.storage.common import UnknownImmutableContainerVersionError, estorage_index_to_checkcode_dir
...
class ImCheckcodeFile(object):
    ...
    def writev(self, datav,new_length):
        ...
        for (offset, data) in datav:
            self._write(f, offset, data)
        f.close()
        
    def _write(self, f,offset, data):
    ...
    # 读取checkcode
    def readv(self,offset, length):
        with open(self.home, 'rb+') as f:
            f.seek(offset)
            data = f.read(length)
        # logging.debug("imcf offset=[{}] length = [{}]".format(offset,length))
        return data
...
class BucketWriter(object):
    ...
    def _allocate_slot_imcheckcode_file(self, ccdir, shn):
        fileutil.make_dirs(ccdir)
        filename = os.path.join(ccdir, "%d" % shn)
        imcheckcode_file = create_immutable_checkcode_file(filename)
        return imcheckcode_file
    
    def write_imcheckcode(self,storage_index,ccodes):
        si_dir = estorage_index_to_checkcode_dir(storage_index)
        ...
        if os.path.isdir(checkcode_dir):
            for sharenum_s in os.listdir(checkcode_dir):
                ...
                mcf = ImCheckcodeFile(checkcode_filename)
                share_imccheckcodes[sharenum] = mcf
        
        for sharenum in ccodes:
            (datav, new_length) = ccodes[sharenum]
            if sharenum not in share_imccheckcodes:
                checkcode_file = self._allocate_slot_imcheckcode_file(checkcode_dir, sharenum)
                share_imccheckcodes[sharenum] = checkcode_file
            share_imccheckcodes[sharenum].writev(datav, new_length)
...
class FoolscapBucketWriter(Referenceable):  # type: ignore # warner/foolscap#78
    ...
    # immutable文件,每个block调用写入challenge block checkcode
    def remote_write_imcheckcode_vectors(self, storage_index, ccodes):
        self._bucket_writer.write_imcheckcode(storage_index,ccodes)
...
def create_immutable_checkcode_file(filename):
    ms = ImCheckcodeFile(filename)
    ms.create()
    del ms
    return ImCheckcodeFile(filename)